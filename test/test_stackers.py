...
def test_sigma_clip_tile_uses_sigma_clip_and_fallback(monkeypatch):
    ...
    out = stackers.sigma_clip_tile(tile_stack, sigma_hi=3.0, min_keep=3, min_keep_frac=0.1, iterations=1)
...
def test_SigmaClippingNoiseStacker_stacking():
    ...
    s = stackers.SigmaClippingNoiseStacker(sigma_hi=3.0, min_keep=2, min_keep_frac=0.5, iterations=1, tile=3)
